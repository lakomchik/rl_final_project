...
class AgarEnv(gym.Env):
            ...
            if self.killed[i] >= 1:
                ...
                if self.killed[i] == 1:
                    info[i]["episode"] = {
                        "r": self.sum_r[i],
                        "r_g": self.sum_r_g[i],
                        "r_g_i": self.sum_r_g_i[i],
                        "hit": self.hit[i],
                        "dis": self.sum_dis / self.s_n,
                    }
                else:
    ...
    def reset(self):
        ...
        while 1:
            ...
            if self.eval:
                ...
                up = min(
                    1.0, max(0.0, (self.total_step - 5e6) / 5e6)
            ...
            for i in range(self.num_agents):
                if (
                    np.sum(observations[i]) == 0.0
                ):  # sometimes the agent dies just after initialization, we should avoid this.
                    success = False
            observations = np.array(observations)
            observations = {
                "t" + str(i): observations[i] for i in range(self.num_agents)
            }
            if success:
                break
        return observations
    ...
    def parse_obs(self, player, id, action=None):
        ...
        n = [
            ...
            10,
            10,
        ]  # the agent can observe at most 10 self-cells, 5 foods, 5 virus, 10 other script agent cells, 10 other outside agent cells
        ...
        s_size_i = [
            ...
            1,
            1,
        ]  # information size of self-cell, food, virus, script agent cell and other outside agent cells.
        ...
        for j in range(10):
            ...
            if j >= 5:
                for i in range(lim):
                    obs_f[
                        base + i
                    ] = 1.0  # reserved function, it can be used for different visibility of policy or value, but I didn't use it.
            else:
                for i in range(lim):
                    obs_f[base + i * s_size_i[j] : base + (i + 1) * s_size_i[j]] = obs[
                        j
                    ][i]
            base += s_size_i[j] * n[j]

        position_x = (
            (player.centerPos.x) / self.server.config.borderWidth * 2
        )  # [-1, 1]
        position_y = (
            (player.centerPos.y) / self.server.config.borderHeight * 2
        )  # [-1, 1]
        ...
        obs_f[-22] = 0  # self.killed[1 - id] != 0
        obs_f[-23] = sum([c.mass for c in player.cells]) / 50
        obs_f[-24] = 0  # sum([c.mass for c in self.agents[1 - id].cells]) / 50
        obs_f[-25] = 0
        obs_f[-26] = 0
        obs_f[-27] = 0
        obs_f[-28] = 0
    ...
    def cell_obs(self, cell, player, id):
        if cell.cellType == 0:
            # player features
            boost_x = (
                cell.boostDistance * cell.boostDirection.x
            ) / self.server.config.splitVelocity  # [-1, 1]
            boost_y = (
                cell.boostDistance
                * cell.boostDirection.y
                / self.server.config.splitVelocity
            )  # [-1, 1]
            radius = cell.radius / 400
            log_radius = np.log(cell.radius / 100)
            position_x = (
                (cell.position.x) / self.server.config.borderWidth * 2
            )  # [-1, 1]
            position_y = (
                (cell.position.y) / self.server.config.borderHeight * 2
            )  # [-1, 1]
            relative_position_x = (
                (cell.position.x - player.centerPos.x)
                / self.server.config.serverViewBaseX
                * 2
            )  # [-1, 1]
            relative_position_y = (
                (cell.position.y - player.centerPos.y)
                / self.server.config.serverViewBaseY
                * 2
            )  # [-1, 1]
            v_x = (
                (cell.position.x - cell.last_position.x)
                / self.server.config.serverViewBaseX
                * 2
                * 30
            )
            v_y = (
                (cell.position.y - cell.last_position.y)
                / self.server.config.serverViewBaseY
                * 2
                * 30
            )
            features_player = [
                ...
                radius,
                log_radius,
                float(cell.canRemerge == True),
                relative_position_x**2 + relative_position_y**2,
                v_x,
                v_y,
                float(cell.radius * 1.15 > player.maxcell().radius),
            ...
            radius = (
                (
                    cell.radius
                    - (
                        self.server.config.foodMaxRadius
                        + self.server.config.foodMinRadius
                    )
                    / 2
                )
                / (self.server.config.foodMaxRadius - self.server.config.foodMinRadius)
                * 2
            )
            log_radius = np.log(
                cell.radius
            ...
            radius = (
                (
                    cell.radius
                    - (
                        self.server.config.virusMaxRadius
                        + self.server.config.virusMinRadius
                    )
                    / 2
                )
                / (
            ...
            position_x = (
                (cell.position.x - self.server.config.borderWidth / 2)
                / self.server.config.borderWidth
                * 2
            )  # [-1, 1]
            position_y = (
                (cell.position.y - self.server.config.borderHeight / 2)
                / self.server.config.borderHeight
                * 2
            )  # [-1, 1]
            relative_position_x = (
                (
                    cell.position.x
                    - player.centerPos.x
                    + 0 * self.server.config.serverViewBaseX / 2
                )
                / self.server.config.serverViewBaseX
                * 2
            )  # [-1, 1]
            relative_position_y = (
                (
                    cell.position.y
                    - player.centerPos.y
                    + 0 * self.server.config.serverViewBaseY / 2
                )
                / self.server.config.serverViewBaseY
                * 2
            )  # [-1, 1]
    ...
    def parse_reward(self, player, id):
        ...
        reward = (
            mass_reward * self.mass_reward_eps
            + kill_reward * self.kill_reward_eps[id]
            + killed_reward * self.killed_reward_eps
        )
        reward2 = (
            mass_reward * self.mass_reward_eps + killed_reward * self.killed_reward_eps
        )
    ...
    def add_cell_geom(self, cell):
        if cell.cellType == 0:
            ...
            cellwall.set_color(
                cell.color.r * 0.75 / 255.0,
                cell.color.g * 0.75 / 255.0,
                cell.color.b * 0.75 / 255.0,
            )
            xform = rendering.Transform()
            ...
            geom.set_color(
                cell.color.r / 255.0, cell.color.g / 255.0, cell.color.b / 255.0
            )
            xform = rendering.Transform()
            geom.add_attr(xform)
            xform.set_translation(cell.position.x, cell.position.y)
            ...
            geom.set_color(
                cell.color.r / 255.0, cell.color.g / 255.0, cell.color.b / 255.0, 0.6
            )
            xform = rendering.Transform()
            geom.add_attr(xform)
            xform.set_translation(cell.position.x, cell.position.y)
