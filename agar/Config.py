class Config:
    def __init__(self):
        self.logVerbosity = (
            4  # Console log level (0=NONE 1=FATAL 2=ERROR 3=WARN 4=INFO 5=DEBUG)
        )
        self.logFileVerbosity = 5  # File log level

        # SERVER
        self.serverTimeout = (
            300  # Seconds to keep connection alive for non-responding client
        )
        self.serverWsModule = "ws"  # WebSocket module= 'ws' or 'uws' (install npm package before using uws)
        self.serverMaxConnections = (
            500  # Maximum number of connections to the server. (0 for no limit)
        )
        self.serverPort = (
            443  # Server port which will be used to listen for incoming connections
        )
        ...
        self.serverBots = 6  # Number of player bots to spawn (Experimental)
        ...
        self.serverChat = (
            1  # Allows the usage of server chat. 0 = no chat 1 = use chat.
        )
        self.serverChatAscii = (
            1  # Set to 1 to disable non-ANSI letters in the chat (english only)
        )
        self.separateChatForTeams = (
            0  # Set to 1 to separate chat for game modes with teams like 'Teams'
        )
        self.serverName = "MultiOgar-Edited #1"  # Server name
        self.serverWelcome1 = (
            "Welcome to MultiOgar-Edited!"  # First server welcome message
        )
        self.serverWelcome2 = (
        ...
        self.serverMinionIgnoreTime = (
            30  # minion detection disable time on server startup [seconds]
        )
        ...
        # VIRUSES
        self.virusMinRadius = (
            100  # Minimum virus radius. (vanilla= mass = val*val/100 = 100 mass)
        )
        ...
        self.virusEqualPopRadius = (
            0  # Whether popped cells have equal radius or not (1 to enable)
        )
        ...
        self.motherCellMaxMass = (
            0  # Maximum amount of mass a mothercell is allowed to have (0 for no limit)
        )
        ...
        self.playerSpeed = (
            1  # Player speed multiplier (1 = normal speed 2 = twice the normal speed)
        )
        ...
        # MINIONS
        self.minionStartRadius = (
            31.6227766017  # Start radius of minions (mass = 32*32/100 = 10.24)
        )
        ...
        self.disableQ = (
            0  # Whether or not to disable Q controls for minions. (Set 0 to enable)
        )
        ...
        self.minionsOnLeaderboard = (
            0  # Whether or not to show minions on the leaderboard. (Set 0 to disable)
        )

        # TOURNAMENT
        self.tourneyMaxPlayers = (
            12  # Maximum number of participants for tournament style game modes
        )
        self.tourneyPrepTime = (
            10  # Number of ticks to wait after all players are ready (1 tick = 1000 ms)
        )
        self.tourneyEndTime = (
            30  # Number of ticks to wait after a player wins (1 tick = 1000 ms)
        )
