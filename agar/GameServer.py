...
class GameServer:
    def __init__(self, env):
        ...
        # Config
        self.config = Config()
        self.food_rate = 0.1  # 0.0 + 0.0 * random.random()
        ...
        if self.run:
            ...
            for cell in self.movingNodes:
                ...
                def callback_fun(check):
                    ...
                    if (
                        cell.cellType == 3
                        and check.cellType == 3
                        and not self.config.mobilePhysics
                    ):
    ...
    def autoSplit(self, cell, player):
        ...
        if len(player.cells) >= self.config.playerMaxCells or self.config.mobilePhysics:
            ...
            self.splitPlayerCell(player, cell, angle, cell.mass * 0.5)
    ...
    def checkRigidCollision(self, m):
        ...
        if m.cell.owner != m.check.owner:
            ...
            return (
                self.gameMode.haveTeams and m.cell.owner.team == m.check.owner.team
    ...
    def resolveRigidCollision(self, m):
        ...
        push = min(
            (m.cell.radius + m.check.radius - m.d) / m.d,
            m.cell.radius + m.check.radius - m.d,
        )
        if push <= 0:
            return
    ...
    def randomPos2(self):
        ...
        pos = Vec2(
            box.minx + random.random() * (box.maxx - box.minx),
    ...
    def spawnCells(self):
        ...
        spawnCount = int(max(min(maxCount, self.config.foodSpawnAmount), minCount))
    ...
    def spawnPlayer(self, player, pos):
        ...
        if player.name == "dummy":
            ...
            radius = self.config.playerStartRadius * (0.4 + 0.2 * random.random())
        ...
        if self.nodesEjected:
            ...
            if (
                random.random() <= self.config.ejectSpawnPercent
                and eject
                and eject.boostDistance < 1
            ):
    ...
    def splitCells(self, player):
        ...
        for cell in cellToSplit:
            ...
            self.splitPlayerCell(player, cell, d.angle(), cell.mass * 0.5)
    ...
    def ejectMass(self, player):
        ...
        for cell in player.cells:
            ...
            pos = Vec2(
                cell.position.x + d.x * cell.radius, cell.position.y + d.y * cell.radius
            )
            angle = d.angle() + (random.random() * 0.6) - 0.3
